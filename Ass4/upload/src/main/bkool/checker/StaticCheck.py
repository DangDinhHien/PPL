...
class StaticChecker(BaseVisitor,Utils):
    ...
    global_envi = [Node("io",[Symbol("readInt",MType([],IntType())),\
    ...
    def visitClassDecl(self, ast, c):
        ...
        for mem in ast.memlist:
            ...
            if type(mem) is AttributeDecl:
                if type(mem.decl) is ConstDecl:
                    kq = self.check_exist(env_class, mem.decl, "Constant")
                    ### check type voi expr
                    ...
                    env_class.append(self.check_exist(env_class, mem.decl, "Variable"))
    ...
    def visitMethodDecl(self, ast, c):
        ...
        for vd in ast.body.decl:
            if type(vd.decl) is ConstDecl:
                #############
                kq = self.check_exist(local_var, vd.decl, "Constant")
                ### check type voi expr
                val = self.visit(vd.decl.value,(env, c[1], c[2], env_class, c[4], c[5]))
                if self.check_type_const(vd.decl.constType, val) is False:
                    raise TypeMismatchInConstant(vd)

                #local_var.append(self.check_exist(local_var, vd.decl, "Constant"))
                local_var.append(kq)
                self.delete_symbol(env_class, vd.decl.constant)
            else:
                local_var.append(self.check_exist(local_var, vd.decl, "Variable"))
                self.delete_symbol(env_class, vd.decl.variable)
        env_class += local_var
    ...
    def visitBlock(self, ast, c):
        ...
        para = []
        for p in ast.decl:
            if type(p.decl) is ConstDecl:
                # para.append(self.check_exist(local_var, p.decl, "Constant"))
                # self.delete_symbol(env_class, p.decl.constant)
                kq = self.check_exist(para, p.decl, "Constant")
                ### check type voi expr
                val = self.visit(p.decl.value,(env, c[1], c[2], env_class, c[4], c[5]))
                if self.check_type_const(p.decl.constType, val) is False:
                    raise TypeMismatchInConstant(p)
                para.append(kq)
                self.delete_symbol(env_class, p.decl.constant)
            else:
                para.append(self.check_exist(para, p.decl, "Variable"))
                self.delete_symbol(env_class, p.decl.variable)  # remove neu trung ko thi thoi
        env_class += para
