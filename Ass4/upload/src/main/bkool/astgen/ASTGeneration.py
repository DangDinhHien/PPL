...
class ASTGeneration(BKOOLVisitor):
    ...
    def visitList_of_members(self,ctx:BKOOLParser.List_of_membersContext):
        ...
        for i in range(0, ctx.getChildCount()):
            ...
            temp = self.visit(ctx.getChild(i))
            if not isinstance(temp,list):
                temp = [temp]
            lst += temp
    ...
    def visitMethod(self, ctx:BKOOLParser.MethodContext):
        ...
        ####################
        if ctx.type_lit():
            returnType = self.visit(ctx.type_lit())
        elif ctx.VOID():
            returnType = VoidType()
        else:
            returnType = VoidType()
            return MethodDecl(temp, Id("<init>"), param, returnType, self.visit(ctx.block_state()))
    ...
    def visitArray_type(self, ctx:BKOOLParser.Array_typeContext):
        #array_type: (ID | primitive_type) LSB (INTEGERLIT) RSB ;
        eleType = self.visit(ctx.primitive_type()) if ctx.primitive_type() else ClassType(Id(ctx.ID(0).getText()))
        ######## ID xem xet lai bi thieu
        size = IntLiteral(int(ctx.INTEGERLIT().getText()))
    ...
    def visitExp9(self, ctx:BKOOLParser.Exp9Context):
        if ctx.getChildCount() == 3:
            return FieldAccess(self.visit(ctx.getChild(0)),self.visit(ctx.getChild(2)))
            #return BinaryOp(ctx.getChild(1).getText(), self.visit(ctx.getChild(0)), self.visit(ctx.getChild(2)))
        else:
            return self.visit(ctx.getChild(0)) 
    ...
    def visitBlock_state(self, ctx:BKOOLParser.Block_stateContext):
        ...
        # att = []
        # for x in range(0,len(ctx.attribute())):
        #     att.append(self.visit(ctx.getChild(x)))
